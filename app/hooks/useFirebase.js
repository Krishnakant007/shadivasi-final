...
//   const getUserData = async (forceRefresh = false) => {
//     if (!user || hasLoaded.current && !forceRefresh) return;
...
export const useFirestore = () => {
  ...
  // ============ NEW DOCUMENT VERIFICATION FUNCTIONS ============

  // Upload verification document
  const uploadVerificationDocument = async (documentType, imageUri) => {
    if (!user || !imageUri) {
  ...
  const getVerificationDocuments = async (status = 'pending') => {
    try {
      const docsRef = collection(db, 'verificationDocuments');
      const q = query(docsRef, where('status', '==', status));
      const querySnapshot = await getDocs(q);
  ...
  const approveVerificationDocument = async (documentId, userId) => {
    try {
  ...
  const rejectVerificationDocument = async (documentId, userId, reason = 'Document not clear') => {
    try {
      // Update document status
      const docRef = doc(db, 'verificationDocuments', documentId);
      await updateDoc(docRef, {
  ...
  const getUserVerificationDocument = async (userId = null) => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        const userData = userDoc.data();
        return userData.verificationDocument || null;
      }
      
      return null;
    } catch (error) {
      console.error("Error getting user verification document:", error);
      throw error;
    }
  };

  // Check if user has submitted verification document
  const hasVerificationDocument = () => {
    return userData?.verificationDocument !== undefined && userData?.verificationDocument !== null;
  };

  // Get verification status
  const getVerificationStatus = () => {
    return userData?.verificationStatus || 'not_submitted';
  };

  // ============ END OF NEW DOCUMENT VERIFICATION FUNCTIONS ============
